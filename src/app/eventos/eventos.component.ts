...
export class EventosComponent implements OnInit {
  ...
  evento = {name:'PHP Conference', date: '6/30/2020', time: '10am', location: {address: 'UniBH ', city: 'Belo Horizonte', country: 'Brasil'}};;
  ...
  receberEnderecoAtualizado(evento){
    this.eventos.forEach((valor)=>{
    if(valor.name===evento.name){
      valor.location=evento.location;
    }
    });
  }
